...
// Inline event handlers the theme itself writes: none at the moment (the hero
// art used to fade in on load; now the picture fades itself in). Anything
// inline is blocked, so a handler that slips past the sanitizer never runs.
// Add a theme handler's exact source here if it ever needs one.
export const THEME_INLINE_HANDLERS = [];
const hash = (s) => `'sha256-${createHash("sha256").update(s).digest("base64")}'`;

export const PAGE_CSP = [
  "default-src 'self'",
  ["script-src 'self'", ...(THEME_INLINE_HANDLERS.length ? ["'unsafe-hashes'", ...THEME_INLINE_HANDLERS.map(hash)] : [])].join(" "),
