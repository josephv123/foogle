...
export function pageFactsPrompt({ url, query, title, snippet, siteContext }) {
  const arrival = [
...
export function pageSectionPrompt({ plan, domain, url, brief, index, total, briefs = [], facts = "", picture = false }) {
  const others = briefs.filter((_, i) => i !== index).map((b) => `- ${b.replace(/\. Topic: [\s\S]*$/, "")}`).join("\n");
  return {
    system: `You write ONE section of the body of a web page that doesn't exist. The page's stylesheet, header and footer already exist and are out of your hands. Make this section look designed, not like a plain document: use the pre-styled components below so it has visual structure — data, people, sequences, cards — the way a real ${plan.kind} site would.
...
${SECTION_CLASSES}
- Plain tags (h2, h3, p, ul, ol, li, table, blockquote, b, i, code, a) are already styled too. The only style="" attributes allowed are the --v and --r values shown above.
