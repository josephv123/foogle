...
// toward compositions that suit the look; `media` are the picture media
// (lib/images.js) that belong on such a page; the rest is seeded per site.
export const STYLES = {
  swiss: {
    ...
    media: ["poster", "flat", "diagram", "photo"],
    fonts: [["helvetica", "helvetica"], ["helvetica", "avenir"], ["arial", "helvetica"]],
    header: ["bar", "minimal", "stacked"], hero: ["poster", "split", "banner"], sections: ["ledger", "numbered"],
    ...
    palette: (h, pick) => {
      ...
      const bg = pick("bg", ["#ffffff", "#f1efe9", "#f4f4f4", hsl(h, 35, 93), "#e9e6dc"]);
...
${pick("grid", [true, false, false]) ? `body{background-image:linear-gradient(90deg,rgba(0,0,0,.05) 1px,transparent 1px);background-size:calc(100% / 12) 100%}` : ""}`,
  },
  ...
  brutalist: {
    ...
    media: ["poster", "collage", "flat", "snapshot"],
...
${c.page ? `main.wrap,.layout,.hero .wrap,footer.site .wrap{background:${c.page}}.hero{border:0}header.site .wrap{background:${c.page}}main input,main textarea,main select{background:#fff}` : ""}
...
${pick("construction", [true, false]) ? `main::before{content:"🚧 This site is under construction! 🚧";display:block;text-align:center;font-weight:700;color:var(--acc2);margin:0 0 20px}` : ""}`,
  ...
  classifieds: {
    about: "a dense utilitarian text site like a classifieds board or a link aggregator: tiny type, plain lists of links, almost no decoration",
    voice: "Dense and text-first, like a classifieds board or a link aggregator: plain ul lists of links with small .meta details, compact tables, terse lines. No .card grids, no .thumb, no .icon, no .bars, no emoji, no pictures.",
    mood: "light", art: "none", mark: false,
    media: ["snapshot", "photo"],
...
footer.site{background:none;border-top:1px solid #ccc;font-size:.85em;padding:10px 0 24px;margin-top:20px}`,
  ...
  editorial: {
    ...
    media: ["photo", "painting", "watercolor", "line"],
...
th{font-family:var(--body)}tbody tr:nth-child(even) td{background:none}.thumb{animation:none;border:0}`,
  ...
  saas: {
    ...
    media: ["isometric", "render3d", "screenshot", "flat", "product"],
...
section>h2{text-align:inherit}.tag{border-radius:999px;background:${hsl(h, 80, c.dark ? 18 : 95)};border-color:transparent}`,
  ...
  scrapbook: {
    ...
    mood: "paper", art: "full", mark: false,
    media: ["collage", "snapshot", "watercolor", "sticker"],
...
footer.site{background:none;border-top:2px dashed var(--line)}`,
  ...
  terminal: {
    ...
    media: ["blueprint", "diagram", "pixel", "line"],
    ...
    palette: (h, pick) => {
      ...
      // A solid panel: the cart drawer and dialogs (public/fw/widgets.css) sit on it.
      const [bg, panel] = pick("bg", [["#050805", "#0b120b"], ["#0c0c0c", "#151515"], ["#101014", "#18181e"]]);
      return { bg, panel, fg, muted: "color-mix(in srgb," + fg + " 60%,#000)", line: "color-mix(in srgb," + fg + " 40%,#000)", acc: fg, acc2: "#ffffff", heroA: "#000", heroB: "#000", on: "#000", art: "#000000" };
    },
...
.announce{background:var(--fg);color:var(--bg);letter-spacing:.2em;text-transform:uppercase;font-weight:400;font-size:.68rem}`,
  ...
  web2: {
    ...
    mood: "light", art: "full", mark: true,
    media: ["flat", "sticker", "render3d", "isometric", "product"],
...
.thumb{animation:none}th{background:linear-gradient(#f5f8fb,#dde6ee);color:#333}`,
  ...
  phpbb: {
    ...
    media: ["snapshot", "photo", "screenshot"],
...
footer.site{background:none;border:0;text-align:center}footer.site .wrap::after{content:"Powered by ${pickForum(c.hh)} © 2004 Group";width:100%;font-size:.8em}`,
  ...
  corporate: {
    ...
    mood: "light", art: "full", mark: true,
    media: ["photo", "flat", "isometric", "product"],
...
footer.site{background:${c.nav};color:rgba(255,255,255,.7);border:0}footer.site a,footer.fat h4{color:rgba(255,255,255,.85)}.thumb{animation:none}`,
  ...
  newsprint: {
    ...
    mood: "paper", art: "full", mark: false,
    media: ["scan", "photo", "line", "map", "chart"],
...
main input,main textarea,main select{background:#fff;color:#000;border-radius:0;box-shadow:inset 1px 1px #808080,inset -1px -1px #fff}
main.wrap,.layout{padding-bottom:20px}footer.site{background:#c0c0c0;box-shadow:var(--bevel);border:0;color:#000}footer.site a{color:#000}`,
  ...
  minimal: {
    ...
    mood: "light", art: "small", mark: false,
    media: ["photo", "line", "watercolor", "snapshot"],
...
.av{background:var(--line);color:var(--fg)}.quote{font-style:normal;border-left:2px solid var(--line)}footer.site{background:none;border:0}`,
  ...
  playful: {
    ...
    mood: "light", art: "full", mark: true,
    media: ["sticker", "flat", "render3d", "product"],
...
footer.site{background:var(--fg);color:#fff;border:0}footer.site a,footer.fat h4{color:#fff}`,
  ...
  cyber: {
    ...
    media: ["neon", "render3d", "pixel", "screenshot"],
...
th{color:var(--acc)}.stat b{text-shadow:0 0 14px ${hsl(h, 100, 60, 0.6)}}`,
  ...
  bento: {
    ...
    mood: "light", art: "full", mark: true,
    media: ["isometric", "render3d", "flat", "product", "chart"],
...
ol.steps li::before{background:var(--fg)}footer.site{background:var(--panel);border-top:1px solid var(--line);color:var(--fg)}footer.site a{color:var(--fg);text-decoration:underline}`,
  ...
  academic: {
    ...
    media: ["diagram", "scan", "line", "chart", "photo"],
...
footer.site{background:none;border-top:1px solid var(--line)}footer.site .wrap::after{content:"Last modified: ${["March", "June", "October", "January"][plan.hue % 4]} ${3 + (plan.hue % 25)}, 2009 · webmaster@${String(plan.site).replace(/[^a-z0-9.-]/gi, "")}";width:100%;font-style:italic}`,
  ...
  vintage: {
    ...
    mood: "paper", art: "full", mark: false,
    media: ["woodcut", "line", "scan", "painting"],
...
.thumb{animation:none;filter:sepia(.8)}hr{border-top:3px double var(--line)}footer.site{background:none;border-top:4px double var(--line);text-align:center}`,
  ...
  devtool: {
    ...
    media: ["isometric", "screenshot", "render3d", "blueprint"],
...
// Jev finds some styles vaguely plausible for almost anything (a minimal or
// Swiss site could be anything) and others rarely (luxury, terminal). These
// are its mean ratings over 24 varied test sites; a style is scored by how
// much more plausible it is *here* than usual, so the generic ones don't win
// every draw. Unlisted styles count as average.
const BASELINE = { swiss: 0.55, brutalist: 0.51, web1996: 0.36, classifieds: 0.33, editorial: 0.33, saas: 0.43, scrapbook: 0.36, terminal: 0.27, luxury: 0.23, web2: 0.47, phpbb: 0.51, corporate: 0.38, newsprint: 0.24, retroos: 0.25, minimal: 0.58, playful: 0.34, cyber: 0.45, bento: 0.44, civic: 0.25, academic: 0.24, vintage: 0.52, devtool: 0.33 };
const AVERAGE = 0.4;

export const STYLE_KEYS = Object.keys(STYLES);

// A well-mixed 32-bit hash. FNV alone won't do here: its low bits depend only
// on the input's low bits, so two unrelated domains can agree on every
// "hash % n" choice and come out as the same design.
const fnv = (s) => createHash("md5").update(String(s)).digest().readUInt32LE(0);
...
// Choose a style from Jev's plausibility ratings: each style's lift over its
// usual rating, discounted for styles the kind rarely wears. Anything close
// to the best score is a candidate, weighted by its score, and the domain
// decides — so the same site always gets the same style, but forums don't
// all become phpBB boards.
export function pickStyle(site, ratings = {}, kind = "blog") {
  const usual = new Set(KIND_STYLES[kind] ?? KIND_STYLES.blog);
  const rated = STYLE_KEYS
    .map((k) => [k, (Number(ratings[k]) - (BASELINE[k] ?? AVERAGE) + AVERAGE) * (usual.has(k) ? 1 : OFF_KIND)])
    .filter(([, p]) => Number.isFinite(p));
  if (!rated.length) return defaultStyle(kind, site);
  const top = Math.max(...rated.map(([, p]) => p));
  const pool = rated.filter(([, p]) => p >= top - 0.25).map(([k, p]) => [k, Math.max(p, 0.01)]);
