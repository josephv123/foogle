...
// Returns { mime, buf }.
export const generateImage = (spec) => drawSVG(imageMessages(spec));

// Any SVG from a { system, user } prompt, in an image slot and sanitized.
// The Foogle Doodle (lib/doodle.js) brings its own prompt.
export function drawSVG(messages, { maxTokens = 2400 } = {}) {
  return withImageSlot(async () => {
    const text = await completeText({ ...messages, maxTokens, temperature: config.tempPages });
