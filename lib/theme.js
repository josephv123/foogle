...
.hero .ctas,.hero .byline,.hero .row{justify-content:center}.hero .art{width:min(560px,100%);aspect-ratio:16/7}`);
  ...
  if (st.hero === "poster") css.push(`.hero-poster .wrap{flex-direction:column;align-items:stretch;gap:28px}.hero-poster .art{width:100%;aspect-ratio:21/8}.hero-poster .tagline{font-size:1.25rem}`);
...
const palette = (plan) => (STYLES[plan.style] ?? STYLES.minimal).palette(plan.hue, siteChooser(plan.site, plan.style));
...
export function themeCSS(plan) {
  ...
  const t = { ...TOKENS, maxw: k.maxw, ...s.tokens(pick) };
  const st = siteStyle(plan);
  const bodyFont = FONTS[st.fonts[0]] ?? FONTS.sans;
  const headFont = FONTS[st.fonts[1]] ?? bodyFont;
...
${STRUCTURE_CSS({ plan, headFont, shadow, st, maxw: t.maxw })}
...
export const artColors = (plan) => {
  const c = palette(plan);
  return `s=${siteImageStyle(plan)}&a=landscape&bg=${encodeURIComponent(c.art ?? c.panel)}&fg=${encodeURIComponent(c.acc)}`;
};
