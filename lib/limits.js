...
export const ACTION_USD = {
  ...
  suggest: 0.00003,
  suggestLong: 0.00025,
  // Extras on a search's first page (lib/answers.js), charged only when they
  // call the model: a card's data (weather, a stock…; $0.00003-0.00021), and
  // the spelling fix behind "Did you mean". Code-only cards (calculator,
  // units, currencies and clocks code knows) are free.
  answer: 0.00025,
  spell: 0.00002,
};
...
export function createLimits({ env = process.env, now = Date.now, maxVisitors = 50_000 } = {}) {
  ...
  // How a request looks to the limiter, for checking a proxy setup (GET
  // /api/whoami): the address and bucket it is charged to, and what the
  // other TRUST_PROXY settings would make of it. It only ever describes the
  // request itself, and reports header names, not values, except these three.
  function whoami(req) {
    ...
    return {
      ...
      forwarded: Object.fromEntries(["x-forwarded-for", "true-client-ip", "cf-connecting-ip"].map((h) => [h, headers[h] ?? null])),
      wouldBe: Object.fromEntries(["1", "2", "true", "cf-connecting-ip", "true-client-ip"].map((t) => [t, clientIP(req, t)])),
    };
  }
  return { allow, check, record, withinBudget, status, whoami, config: { burstUsd: burst, usdPerHour: perHour, dailyBudgetUsd: daily } };
}
