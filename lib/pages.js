import { config, streamText, TruncatedError } from "./llm.js";
...
import { knownSite, learnedSite, learningSite, learnSite } from "./brands.js";
import { brandPlan } from "./brandtheme.js";
...
// A section cut off by the token limit keeps its whole blocks (see blockEnd)
// and is closed there. Returns "" if nothing besides its heading was whole.
...
// A real, well-known site looks like itself (lib/brands.js). Curated ones
// are known at once; for any other the brand spec is asked for only when the
// results page said the site is real or Jev thinks so, and the header waits
// for it at most this long. Invented sites never wait.
const REAL_SITE = 0.6;
const BRAND_WAIT_MS = 2500;
...
export async function* generatePage(args, { planWithJev = jevPlan, stream = streamText, complete, looks = siteLooks, facts: factsFor = pageFacts, factsWaitMs = FACTS_WAIT_MS, site = knownSite, learn = learnSite, learned = learnedSite, learning = learningSite, brandWaitMs = BRAND_WAIT_MS } = {}) {
  const domain = new URL(args.url).hostname;
  let brand = site(domain) ?? learned(domain);
  // The fact sheet generates while the layout is planned and the shell streams.
  const sheet = factSheet(factsFor({ ...args, brand }));
  const look = brand ? null : looks.get(domain);
  // The results page said this is a real site (and started its spec): use
  // that alongside Jev.
  const spec = !brand && !look ? (args.real ? learn(domain) : learning(domain)) : null;
  let plan;
  if (brand) plan = brandPlan(args, brand);
  else if (look) plan = { ...planFromAnswers(args, { kind: { choice: look.kind }, style: { choice: look.style } }), ...look };
  else {
    ...
    if (spec || plan.realSite >= REAL_SITE) {
      brand = await within(spec ?? learn(domain), brandWaitMs);
      if (brand) plan = brandPlan(args, brand);
    }
  }
  if (!look && !brand) {
  ...
  yield `<!DOCTYPE html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1">${themeCSS(plan)}${widgetHead()}</head><body>${themeHeader(plan, domain, { art: true })}`;
  ...
  for (const [index, job] of jobs.entries()) {
    ...
    if (error instanceof TruncatedError) {
      ...
      if (kept && kept.length >= sent) {
        console.warn(`[page] section ${index + 1} of ${args.url} hit the token limit; kept its whole blocks`);
