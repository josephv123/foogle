...
import { designFaviconSVG } from "./design.js";
import { addressOf, displayURL, isHome, tabTitle, omniboxTarget, svgImage, FOOGLE_ICON } from "../public/fw/browsing.js";
...
function faviconSVG(url) {
  ...
  // A real site shows the mark its design spec draws (lib/design.js).
  const spec = siteSpecs.spec(domain);
  if (spec?.look) return designFaviconSVG(spec);
...
const PAGES = ["/", "/search", "/images", "/news", "/maps", "/timelines", "/doodles", /^\/web\//];
...
export function browserBarRoutes() {
  ...
  // For the script: a tab's favicon.
