...
// What a picture's place on the page shows until the picture arrives: its
// colours, out of focus — a soft gradient and two blurred patches. The colours
// are the light, colour scheme or site palette the brief gives the model, or
// the medium's own, so the picture fades in over something close to it.
...
// The sketch as a CSS background for the picture's <img>, so it shows the
// moment the tag is written, before the browser has even asked for the
// picture. The picture then fades in over it (see fadeIn).
...
// The sketch for an /img src a page builds itself: its description and its
// query (s=&a=&bg=&fg=), as /img will read them.
export const sketchFor = (description, query) =>
  sketchCSS(imageSpec(description.trim(), Object.fromEntries(new URLSearchParams(query))));

// A picture fades in as it arrives, over whatever is behind its <img> (the
// sketch), so it goes from the sketch to the finished picture in one smooth
// step. The fade is in the SVG itself, so it needs no script and works
// wherever the picture is shown: every browser runs a CSS animation inside an
// SVG image, starting from its first frame. Chrome runs it only while the
// picture is on screen, so one that finished further down the page fades in
// as it's scrolled to.
export const FADE_IN = `<style>:root{animation:foogle-in .35s ease-out both}@keyframes foogle-in{from{opacity:0}}</style>`;
export const fadeIn = (svg) => String(svg).replace(/<svg\b(?:[^>"']|"[^"]*"|'[^']*')*>/, (open) => open + FADE_IN);
