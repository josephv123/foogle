...
- **A browser bar across the top of every page** (`lib/browserbar.js`, `public/fw/browserbar.{js,css}`): a tab with the page's favicon and title, back / forward / reload, and an address bar showing the URL a real browser would — `https://crumbforum.net/threads/starter-smells` for `/web/crumbforum.net/threads/starter-smells`, `https://www.foogle.com/search?q=…` for Foogle's own pages. Typing a URL (`example.com/foo`) goes to that fake site; anything else is a Foogle search (`GET /go?q=…`). It is in the first bytes of every response, inside a declarative shadow root no site's styles can reach, and the page scrolls below it, so sticky site headers stick under it. On phones it collapses to one row. Set `FOOGLE_BROWSER_BAR=0` to turn it off.
