...
export async function* generatePage(args, { planWithJev = jevPlan, stream = streamText, complete, looks = siteLooks, facts: factsFor = pageFacts, factsWaitMs = FACTS_WAIT_MS } = {}) {
  ...
  let plan;
  if (look) plan = { ...planFromAnswers(args, { kind: { choice: look.kind }, style: { choice: look.style } }), ...look };
  else {
  ...
  const jobs = selected.map((brief, index) => sectionJob((async function* () {
    ...
    yield* untilClosed(write({
      ...pageSectionPrompt({ plan, domain, url: args.url, brief, index, total: selected.length, briefs: selected, facts, picture: allowsPicture(index, selected.length), submission: submission?.summary }),
