...
// ---------- brand specs ----------
// A real site the table doesn't know (lib/brands.js): a plausible look for
// the domain, in the JSON the prompt asks for. Invented-looking domains
// (.example, .test) are "not a site I know".
function brandSpec(system, user) {
  const domain = line("Domain: ", user).toLowerCase();
  if (!domain || /\.(example|test|invalid|local)$/.test(domain)) return JSON.stringify({ known: false });
  const r = rng(`brand|${domain}`);
  const name = titleCase(domain.replace(/^www\./, "").split(".")[0]);
  const hue = r(360);
  const dark = r(3) === 0;
  const hex = (l) => { const k = (n) => (n + hue / 30) % 12, a = 0.6 * Math.min(l, 1 - l), f = (n) => l - a * Math.max(-1, Math.min(k(n) - 3, 9 - k(n), 1)); return `#${[f(0), f(8), f(4)].map((x) => Math.round(x * 255).toString(16).padStart(2, "0")).join("")}`; };
  return JSON.stringify({ known: true, name, arch: pick(r, ["marketplace", "community", "landing", "video", "newsportal"]), bg: dark ? "#141414" : "#ffffff", fg: dark ? "#f2f2f2" : "#161616", accent: hex(0.48), header: dark ? "#141414" : "#ffffff", headerText: dark ? "#f2f2f2" : "#161616", font: pick(r, ["sans", "helvetica", "georgia", "rounded"]), logo: name, logoCase: "none", logoColor: hex(0.45), logoWeight: 800, nav: shuffled(r, ["Discover", "Popular", "New", "Lists", "Shop", "Community", "Help"]).slice(0, 5), tagline: `${name}, the way you like it.`, voice: `friendly, specific copy about ${name}'s own things` });
}
...
// query's own words, and whether it holds a known misspelling.
const CARD_WORDS = [
  ["none", /\b(recipes?|kits?|movies?|deals?|balloons?|history of)\b/],
  ...
  ["time", /^(what )?time (is it )?in\b|\blocal time\b/],
  ["currency", /\b(usd|eur|gbp|jpy|dollars?|euros?|yen|credits?|exchange rate)\b/],
  ["units", /\d\s*(miles?|km|kg|lbs?|pounds|feet|ft|cm|inches|cups?|liters?|litres?|f|c)\s+(to|in)\b/],
  ["calculator", /^[\d\s.+\-*/^()%x×÷]+$|\d+\s*%\s*of\s*\d|\bsqrt\b|\btimes\b/],
];
...
const ROUTES = [
  ...
  [/visual identity of a real, well-known website/, brandSpec],
