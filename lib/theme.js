...
import { siteImageStyle, sketchFor } from "./images.js";
import { STYLES, STYLE_FONTS, SECTION_LAYOUTS, siteChooser, defaultStyle } from "./styles.js";
import { designPalette, designCSS, designHeader, designFooter, designFacts, designTokens, designFonts, designMedium } from "./design.js";
...
export function normalizePlan(raw = {}) {
  ...
  return {
    ...
    nav,
    secs,
    // The page archetype (lib/design.js) when it isn't the kind's usual
    // page: a forum's feed, a news front page, a product page, a wiki's main page.
    page: ["feed", "front", "product", "main"].includes(raw.page) ? raw.page : null,
  };
}
...
function STRUCTURE_CSS({ plan, headFont, shadow, st, maxw }) {
  const css = [`
.brand{display:inline-flex;align-items:center;gap:10px;min-width:0;max-width:100%}.brand .mark{flex:none;border-radius:0}
.brand-text{display:flex;flex-direction:column;line-height:1.1;min-width:0}.brand-text>div{overflow-wrap:normal;overflow-x:clip;text-overflow:ellipsis;padding-right:.2em;margin-right:-.2em}
...
footer.fat .cols{display:grid;grid-template-columns:minmax(0,1.4fr) repeat(3,minmax(0,1fr));gap:28px;width:100%}
...
@media(max-width:640px){header.site .wrap{grid-template-columns:minmax(0,1fr) auto;row-gap:12px}header.site .brand{grid-column:1/-1}header.site nav.top{grid-row:2}header.site .actions{grid-column:2;grid-row:2}}`);
...
@media(max-width:860px){.layout{grid-template-columns:minmax(0,1fr)}.layout>main,.layout>.rail{grid-column:1;grid-row:auto}.rail{position:static;padding-top:0}}`);
  css.push(`@media(max-width:640px){header.site .top-row .search,header.site .actions .search{display:none}footer.fat .cols{grid-template-columns:repeat(2,minmax(0,1fr))}}`);
  return css.join("\n");
}
...
function heroDetails(plan) {
  ...
  // A product page's price: the one its title or snippet states, if any.
  d.price = Number(stated(/\$\s?(\d[\d,]*(?:\.\d\d)?)/)?.replace(/,/g, "")) || [19, 24, 38, 45, 59, 72, 89, 115, 140, 185][r(10)];
...
export function heroFacts(plan) {
  if (plan.design) return designFacts(plan);
  const d = heroDetails(plan);
  if (plan.page) return {
    "forum/feed": [`The forum has ${d.members} members; ${d.online} online now`],
    "store/product": [`${plan.title || "This product"} costs $${d.price}; rated ${d.rating} from ${d.reviews} reviews`, `Free shipping over $${d.shipOver}; ${d.returnDays}-day returns`],
  }[`${plan.kind}/${plan.page}`] ?? [];
  return {
...
function chrome(plan, domain) {
  ...
  // A page that isn't the kind's usual one (see normalizePlan) has its own.
  switch (`${plan.kind}/${plan.page}`) {
    case "forum/feed":
      return {
        pre: `<div class="crumbs"><a href="${link("Forums")}">Forums</a> › Latest</div>`,
        meta: `<div class="byline"><span><b>${d.members}</b> members</span><span>· <b>${d.online}</b> online now</span></div><div class="ctas"><a class="btn" href="${link("New thread")}">Start a thread</a><a class="btn ghost" href="${link("Unanswered")}">Unanswered</a></div>`,
      };
    case "store/product":
      return {
        above: `<div class="announce">Free shipping over $${d.shipOver} · ${d.returnDays}-day returns</div>`,
        pre: `<div class="crumbs"><a href="${link("Shop")}">Shop</a> › Product</div>`,
        meta: `<div class="byline"><span class="stars" style="--r:${d.rating}"></span><span>${d.rating} from ${d.reviews} reviews</span></div><div class="ctas"><span class="price">$${d.price}</span><button class="btn" type="button" data-add-to-cart data-name="${esc(plan.title || plan.site)}" data-price="${d.price}">Add to cart</button></div>`,
      };
    case "news/front":
      return { above: `<div class="dateline"><div class="wrap"><span>${d.date}</span><span><i class="live"></i>Live updates</span></div></div>` };
    case "wiki/main":
      return { sub: `<div class="meta">Welcome to ${site}, the free encyclopedia anyone can edit</div>`, meta: `<div class="wtabs"><a class="on" href="${link("Main page")}">Main page</a><a href="${link("Talk")}">Talk</a><span>Read</span></div>` };
  }
  switch (plan.kind) {
    case "forum":
      return {
...
// A real site (lib/design.js) has its own colours instead.
const palette = (plan) => (plan.design ? designPalette(plan.design) : (STYLES[plan.style] ?? STYLES.minimal).palette(plan.hue, siteChooser(plan.site, plan.style)));
...
export function themeCSS(plan) {
  ...
  const t = { ...TOKENS, maxw: k.maxw, ...(plan.design ? designTokens(plan) : s.tokens(pick)) };
  const st = siteStyle(plan);
  const [bodyFont, headFont] = plan.design ? designFonts(plan) : [FONTS[st.fonts[0]] ?? FONTS.sans, FONTS[st.fonts[1]] ?? FONTS[st.fonts[0]] ?? FONTS.sans];
...
body{background:var(--bg);color:var(--fg);font-family:${bodyFont};line-height:${t.lh};font-size:${t.fs}px;-webkit-font-smoothing:antialiased;overflow-wrap:break-word}
...
.hero .art{flex:none;width:min(340px,38%);min-width:0;height:auto;aspect-ratio:4/3;object-fit:contain;background:${c.panel};border:var(--bw) solid var(--line);border-radius:var(--rad);box-shadow:${shadow}}
...
.split{display:grid;grid-template-columns:minmax(0,1.15fr) minmax(0,1fr);gap:28px;align-items:center;margin:0 0 1.2em}
...
${plan.design ? designCSS(plan, { c, bodyFont, headFont }) : `${STRUCTURE_CSS({ plan, headFont, shadow, st, maxw: t.maxw })}
${s.mark ? "" : ".mark{display:none}"}
${s.css({ c, h: plan.hue, plan, pick, st })}`}
@media(max-width:640px){nav.top{margin-left:0;gap:14px;font-size:.82rem}.hero{padding:30px 0 26px}.hero .wrap{flex-direction:column;align-items:stretch}.hero .art{width:100%}.split{grid-template-columns:minmax(0,1fr)}.infobox,table.infobox{float:none;width:auto;margin:0 0 1em}}
</style>`;
}
...
export const artColors = (plan, { shape = "landscape" } = {}) => {
  ...
  const medium = plan.design ? designMedium(plan) : !media || media.includes(suggested) ? suggested : siteChooser(plan.site, plan.style)("media", media);
...
export function themeHeader(plan, domain, { art = false } = {}) {
  if (plan.design) return designHeader(plan, domain);
  const st = siteStyle(plan);
  const name = plan.site || domain;
  const link = (label) => navHref(domain, label);
  // Put the accent colour on the last word, or on the TLD when the site is
  // named by its domain. On a narrow screen a wordmark wraps between words,
  // after a hyphen or before a dot, never between letters (see .brand-text).
  const parts = name.split(/\s+/);
  const tld = parts.length === 1 && name.match(/^(.+?)(\.[a-z.]+)$/i);
  const dots = (s) => esc(s).replaceAll(".", "<wbr>.");
  const brand = tld
    ? `${dots(tld[1])}<span>${dots(tld[2])}</span>`
    : parts.length > 1
  ...
  // Low-budget styles (a classifieds board, a government form) have no hero
  // art. The art shows its sketch until the picture fades in over it.
  const artQuery = artColors(plan, { shape: WIDE_HEROES.has(st.hero) ? "wide" : "landscape" });
  const heroArt = art && STYLES[plan.style]?.art !== "none"
    ? `<img class="art" src="/img/${imgPath(heroImagePrompt(plan))}?${esc(artQuery)}" style="background:${esc(sketchFor(heroImagePrompt(plan), artQuery))}" alt="">`
    : "";
...
export function themeFooter(plan, domain) {
  if (plan.design) return designFooter(plan, domain);
  const st = siteStyle(plan);
  const link = (label) => navHref(domain, label);
  const name = plan.site || domain;
  const year = 2040 + (plan.hue % 9);
