...
Invent every site — never a real, well-known domain (no real social network, encyclopedia, marketplace, video platform or newspaper): make up the forum, the wiki, the store and the video site that would exist instead, with names that feel lived-in. Invent realistic domains (varied TLDs) and URL paths. Titles under 60 chars. Every result must be a distinct site, and no two results should say the same thing.`;

// The first batch of page 1 decides whether the query is navigational: if it
// names a real, well-known site (or a page on one), that site becomes its
// first line, marked real, with Google-style sitelinks. The results page
// ranks it first and starts writing the site's design spec at once (see
// lib/sitespec.js). No list of real sites exists anywhere: the model knows.
export const REAL_SITE_RULE = `
One exception: if the query itself names or navigates to ONE real, well-known website, or a page on one (the site's name, nickname or domain, alone or with what to find there), make your FIRST line that site's real page and add "real": true to it:
{"site": "its brand name", "title": "the page's own title, as the site writes it", "url": "its real canonical URL, in the site's real URL scheme", "snippet": "its own description", "real": true, "color": "#rrggbb, its brand colour", "links": [["Label", "/path", "one short line about it"], …]}
For the site itself (its name alone, or with words like login, website, news, home), "links" holds up to 6 sitelinks to its main sections. For a page on it (its name plus a topic), the url is that page — a community, an article, a product search, a tag — and "links" holds up to 3 related pages on it. Only for a query that targets the site itself, never for a topic a site merely covers. Every other line is an invented site as usual.`;
...
export function searchShardPrompt(query, { count, angle, page = 1, real = false }) {
  const realRule = real ? REAL_SITE_RULE : "";
  return {
...
export function pageFactsPrompt({ url, query, title, snippet, siteContext, submission, site, real }) {
  const arrival = [
  ...
  return {
    system: `You prepare the fact sheet for ONE web page on ${real ? `${site || "a real, well-known website"} (the real site at this URL), as it will look in the 2040s — invented, futuristic content in its own style` : "a site that doesn't exist"}. Several writers will write the page's sections in parallel from your sheet, so it must settle everything they would otherwise invent differently.
...
export function pageSectionPrompt({ plan, domain, url, brief, index, total, briefs = [], facts = "", picture = false, submission = "" }) {
  ...
  // A real, well-known site (lib/design.js): the writer imitates it.
  const brand = plan.design && designPrompt(plan, domain);
