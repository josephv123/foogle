...
// drawn in the site's colours, and each <img> shows its picture's sketch
// until the picture fades in over it (kept under any style the model gave
// it, so the model's own background wins). A src is only rewritten once its
// closing quote has arrived, and a tag once it has closed; see heldBack().
const IMG_TAG = /<img\b(?:[^<>"']|"[^"]*"|'[^']*')*>/gi;
export function paintPictures(html, plan) {
  const palette = artColors(plan, { shape: plan.picShape }).replaceAll("&", "&amp;");
  const decode = (raw) => { try { return decodeURIComponent(raw).trim(); } catch { return raw; } };
  return html
    .replace(/src="\/img\/([^"?]*)"/g, (_, raw) => `src="/img/${encodeURIComponent(decode(raw).slice(0, 300))}?${palette}"`)
    .replace(IMG_TAG, (tag) => {
      const src = tag.match(/\ssrc="\/img\/([^"?]*)\?([^"]*)"/);
      const style = tag.match(/\sstyle\s*=\s*(["']?)/);
      if (!src || (style && !style[1])) return tag;
      // Only whitelisted media and shapes and checked colours reach the CSS.
      const sketch = `background:${sketchFor(decode(src[1]), src[2].replaceAll("&amp;", "&"))}`;
      if (style) return tag.replace(style[0], () => `${style[0]}${sketch};`);
      return tag.replace(src[0], () => `${src[0]} style="${sketch}"`);
    });
}

// How much of a streaming section is safe to send: nothing from the first
// backtick (it may open a closing code fence), from a picture src whose
// closing quote hasn't arrived or from an <img> that hasn't closed (painting
// will rewrite them).
function heldBack(body, from) {
  ...
  const img = body.lastIndexOf("<img");
  if (img !== -1 && !new RegExp(IMG_TAG.source, "iy").test(body.slice(img))) end = Math.min(end, img);
  return end;
}
