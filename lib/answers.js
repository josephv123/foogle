...
// any, and how likely the query is to hold a typo. Most queries get neither
// and cost only that request, which nothing waits on. When a card is coming
// its slot is sized at once, usually long before the first result lands, so
// the results don't move when the card fills in.
//
...
// browser). The model only fills a small typed JSON object for the rest
// (weather, scores, stocks, definitions, flights, invented currencies), and
// code checks and completes it here before lib/answer-cards.js draws it, so
// a card always agrees with itself: the score is the sum of the quarters,
// today's high is above the current temperature, the chart ends at the price.
...
// Jev's choice needs this probability before a card is shown, and its typo
// estimate this much before the spelling is checked. Measured on a few dozen
// queries: real cards score ~1.0, lookalikes ("stock pot recipes") ~0;
// misspellings score 0.84-0.99, invented names and terse queries under 0.25.
...
export function createAnswers({ complete, classify, charge = () => true, now = () => new Date(), fillTimeoutMs = 12_000, spellTimeoutMs = 6000, log = console } = {}) {
  ...
  return {
    ...
    async start(query, { req, emit, shown = () => false }) {
      ...
      if (jev || type) log.log(`[answers] "${query}" → ${type ?? "no card"}${jev ? ` (${jev.type} ${jev.p.toFixed(2)}, typo ${jev.typo.toFixed(2)})` : " (code)"} +${((Date.now() - t0) / 1000).toFixed(2)}s`);
      ...
      const card = async () => {
        ...
        try {
          const raw = own ?? await model(answerPrompt(type, query, { now: at }), answerMaxTokens(type), fillTimeoutMs, `${type} card`);
          const html = RENDER[type](NORMALIZE[type](raw, { query, now: at }), query);
          send(fill("ia", html, `ld${own ? grow : ""}`) + ANSWER_JS);
          log.log(`[answers] ${type} card +${((Date.now() - t0) / 1000).toFixed(2)}s`);
        } catch (err) {
          log.warn(`[answers] ${type} card: ${err.message}`);
          if (!own) emit(clear("ia"));
        }
      };
