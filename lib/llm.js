...
// prompt. `ticket` is the picture's place in the queue; set `ticket.urgent`
// while it waits to move it up.
export function drawSVG(messages, { maxTokens = 2400, ticket = { urgent: true } } = {}) {
  return withImageSlot(async () => {
    let text = "";
    try {
      for await (const delta of streamText({ ...messages, maxTokens, temperature: config.tempPages, provider: FAST_ROUTE })) text += delta;
    } catch (err) {
