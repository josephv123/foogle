...
// What a picture shows from the moment it's asked for until the model's first
// shapes arrive (see /img in server.js): its colours, out of focus — a soft
// gradient and two blurred patches — with a slow sheen crossing it. The
// colours are the light, colour scheme or site palette the brief gives the
// model, or the medium's own, so the first shapes land on something close.
// [top, bottom, patch, patch]
const LIGHT_TONES = {
  "bright midday sun, crisp shadows, saturated colour": ["#7fbfea", "#d8d0ac", "#5c9a4f", "#f4ead0"],
  "warm golden-hour side light": ["#f3b86d", "#8a5a3c", "#f9dba2", "#5b3f35"],
  "soft overcast daylight, muted colour": ["#c9d0d4", "#8e978f", "#aab3ad", "#e3e6e4"],
  "blue-hour dusk with glowing windows": ["#2b3a66", "#141a2e", "#f1b45a", "#5a6aa0"],
  "night under orange streetlights": ["#1a1b28", "#3b2e2a", "#e89a3c", "#2e3450"],
  "harsh on-camera flash": ["#2b2a2c", "#8f8378", "#efe7dd", "#4a4548"],
  "cool fluorescent indoor light": ["#dfe8e6", "#9fb0ad", "#c7d6d3", "#7d8f8c"],
  "hazy backlight with a bright rim": ["#f6e7c6", "#9c8a70", "#fff6e0", "#c8b08a"],
  "clear morning light, fresh and airy": ["#bfe0f2", "#e6eee0", "#9ccf8e", "#ffffff"],
  "warm tungsten lamps indoors": ["#5e3f28", "#2c1f18", "#e0a15a", "#8a5a36"],
};
const SCHEME_TONES = {
  "teal, coral and cream": ["#f6efe0", "#e9dcc4", "#2a9d8f", "#f28c70"],
  "navy, mustard and off-white": ["#f7f4ec", "#e6e0d0", "#23395d", "#e1ad3b"],
  "forest green, peach and ivory": ["#fbf7ea", "#ece4cf", "#2f5d3a", "#f6b99a"],
  "violet, lime and charcoal": ["#3a3d42", "#26282c", "#7a4fd0", "#b6e04a"],
  "tomato red, sky blue and sand": ["#8ecae6", "#e9d8b4", "#e5533d", "#f4ead2"],
  "black, hot pink and white": ["#ffffff", "#ececec", "#151515", "#ff3e9a"],
  "ochre, olive and rust": ["#e9d9b0", "#c9962b", "#6b7a2f", "#a94a26"],
  "cobalt, orange and pale grey": ["#e9ecef", "#d5dae0", "#1f4fbf", "#f28a1e"],
  "mint, plum and butter yellow": ["#fbeeb8", "#a8e6cf", "#7b3f6e", "#fff7d6"],
  "slate, gold and blush": ["#f3d9d1", "#4a5664", "#d4a537", "#f7e8e2"],
  "emerald, magenta and cream": ["#f6efe0", "#e6dcc6", "#1f9d6b", "#d1348f"],
  "sepia browns with one turquoise": ["#d9c3a0", "#7a5a3a", "#2ec4b6", "#b08b5e"],
};
// Media with colours of their own, whatever the scheme.
const MEDIUM_TONES = {
  product: ["#f5f5f3", "#d6d6d2", "#ffffff", "#bdbdb8"],
  line: ["#f6f2e9", "#ede6d6", "#d8cfbd", "#fbf8f1"],
  woodcut: ["#efe6d2", "#e2d5b9", "#3a322b", "#f6efdf"],
  watercolor: ["#fbfaf6", "#efece4", null, null], // washes of the light's colours
  blueprint: ["#1f4f8f", "#163a6a", "#3b73b6", "#0f2c55"],
  screenshot: ["#f7f8fa", "#e7ebf0", "#cfd8e3", "#ffffff"],
  map: ["#eef1e2", "#d9e6cf", "#a8cfe0", "#f2e3b3"],
  neon: ["#1c0b35", "#07040f", "#ff2bd6", "#19d9ff"],
  scan: ["#e5d5b5", "#b39469", "#f3e9d3", "#8a6d49"],
};
const OWN_COLOURS = new Set(["blueprint", "neon"]); // even on a site

function tones({ description, style, palette }) {
  if (OWN_COLOURS.has(style)) return MEDIUM_TONES[style];
  const light = LIGHT_TONES[pick(LIGHTS, description)];
  if (LIT.has(style) || style === "meme") {
    const [top, bottom, a, b] = MEDIUM_TONES[style] ?? light;
    return [top, bottom, a ?? light[0], b ?? light[2]];
  }
  if (palette) return [palette.bg, palette.bg, palette.fg, palette.bg];
  if (STYLES[style].free) return SCHEME_TONES[pick(COLOR_SCHEMES, `${style} ${description}`)];
  return MEDIUM_TONES[style] ?? light;
}

// Where the two patches sit, in percent of the canvas.
function sketchLayout(description) {
  const r = hash(`sketch ${description}`);
  const at = (bits, lo, span) => lo + ((r >>> bits) % span);
  return { a: [at(0, 15, 40), at(6, 35, 45)], b: [at(12, 55, 35), at(18, 20, 50)] };
}

export function sketchSVG(spec) {
  const [w, h] = SHAPES[spec.shape];
  const [top, bottom, a, b] = tones(spec);
  const { a: [ax, ay], b: [bx, by] } = sketchLayout(spec.description);
  const x = (pct) => Math.round((pct / 100) * w);
  const y = (pct) => Math.round((pct / 100) * h);
  const sheen = Math.round(w * 0.6);
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${w} ${h}"><defs>`
    + `<linearGradient id="b" x2="0" y2="1"><stop stop-color="${top}"/><stop offset="1" stop-color="${bottom}"/></linearGradient>`
    + `<radialGradient id="p"><stop stop-color="${a}" stop-opacity=".6"/><stop offset="1" stop-color="${a}" stop-opacity="0"/></radialGradient>`
    + `<radialGradient id="q"><stop stop-color="${b}" stop-opacity=".5"/><stop offset="1" stop-color="${b}" stop-opacity="0"/></radialGradient>`
    + `<linearGradient id="s" x2="1"><stop stop-color="#fff" stop-opacity="0"/><stop offset=".5" stop-color="#fff" stop-opacity=".22"/><stop offset="1" stop-color="#fff" stop-opacity="0"/></linearGradient>`
    + `</defs><rect width="${w}" height="${h}" fill="url(#b)"/>`
    + `<ellipse cx="${x(ax)}" cy="${y(ay)}" rx="${Math.round(w * 0.45)}" ry="${Math.round(h * 0.4)}" fill="url(#p)"/>`
    + `<ellipse cx="${x(bx)}" cy="${y(by)}" rx="${Math.round(w * 0.35)}" ry="${Math.round(h * 0.35)}" fill="url(#q)"/>`
    + `<rect x="${-sheen}" width="${sheen}" height="${h}" fill="url(#s)"><animate attributeName="x" from="${-sheen}" to="${w}" dur="1.8s" repeatCount="indefinite"/></rect>`
    + `</svg>`;
}

// The same sketch as a CSS background, for a search result to show the
// moment it's written, before the browser has even asked for its picture.
export function sketchCSS(spec) {
  const [top, bottom, a, b] = tones(spec);
  const { a: [ax, ay], b: [bx, by] } = sketchLayout(spec.description);
  const fade = (c, pct) => `color-mix(in srgb, ${c} ${pct}%, transparent)`;
  return `radial-gradient(45% 40% at ${ax}% ${ay}%, ${fade(a, 60)}, transparent), radial-gradient(35% 35% at ${bx}% ${by}%, ${fade(b, 50)}, transparent), linear-gradient(${top}, ${bottom})`;
}
